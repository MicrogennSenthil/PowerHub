...
- **Occupancy sync authentication**: M-HMS resolves `X-API-Key` against exactly one enabled hotel. A current Power Automation row is authoritative whenever it exists; replaced/disabled keys never fall back to legacy. Legacy keys work only for hotels without a current row, and duplicate effective matches fail closed.
- **Occupancy mapping**: active guest/Occupied → Checkin; Cleaning → Cleaning; Visiting → Visiting; maintenance/repair/service → Maintenance; everything else → Vacant. Checkin, Cleaning, Visiting, and Maintenance keep power ON; Vacant turns it OFF.
